//! Kani harnesses for [`BigFloat::log10`].
//!
...
fn log10_nan_propagates() {
    ...
    let (r, _status) = a.log10(RoundingMode::NearestEven);
    assert!(r.is_nan());
}

#[kani::proof]
fn log10_signaling_nan_raises_invalid() {
    let a = BigFloat::try_new_signaling_nan(Sign::Positive, 53, &[]).expect("precision >= 1");
    let (r, status) = a.log10(RoundingMode::NearestEven);
    assert!(r.is_nan());
    assert!(status.contains(Status::INVALID));
}

#[kani::proof]
fn log10_pos_zero_is_neg_inf_div_by_zero() {
...
#[kani::proof]
fn log10_neg_finite_is_nan_invalid() {
