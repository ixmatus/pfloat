//! Kani harnesses for [`BigFloat::tanh`].
//!
//! `tanh(NaN) = NaN`, `tanh(±∞) = ±1`, `tanh(±0) = ±0`.
...
fn tanh_nan_propagates() {
    ...
    let (r, _status) = a.tanh(RoundingMode::NearestEven);
    assert!(r.is_nan());
}

#[kani::proof]
fn tanh_signaling_nan_raises_invalid() {
